...
def SectionBOX4(formulaStr, formula): # formulaStr es la que se muestra, formula es la q se analiza
   ...
   if puntos_criticos:
       ...
       for punto, tipo in puntos_criticos:
        ...
        interpretacion_puntos += (f"$t = {latex_punto}$ es un {tipo}, "f"{'en este punto el costo de mantención del servidor alcanza un nivel bajo, pero comienza a aumentar.' if tipo == 'mínimo' else 'el costo de mantención alcanza un nivel alto, pero luego comienza a disminuir, se recomienda analizar cuidadosamente las estrategias de mantenimiento.'} ""<br />")
   else:
